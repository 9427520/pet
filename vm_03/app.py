    ...
    if postback_data == "毛小孩健保卡":
        ...
        if not pets:
            ...
            if not selected_pet:
                ...
                message = TextMessage(text="請用直拍方式拍攝寵物的健康檢查報告。", sender=sender, quick_reply=quick_reply)
                with ApiClient(configuration) as api_client:
                    line_bot_api = MessagingApi(api_client)
        ...
        if action == "view_history":
            ...
            pet_name = data.get('p_n')
            report_date = report_name.replace(f'{pet_name}_', '').replace('.png', '').replace('.jpg', '')
            quick_reply=QuickReply(items=[
                QuickReplyItem(action=PostbackAction(label="確定", data=json.dumps({"action": "confirm_delete_report", "report_name": report_name, "p_n": pet_name}))),
                QuickReplyItem(action=PostbackAction(label="取消", data=json.dumps({"action": "cancel_delete"})))
            ])

            message = TextMessage(text=f"您確定要刪除報告 {report_date} 嗎？", sender=sender, quick_reply=quick_reply)
            with ApiClient(configuration) as api_client:
                    line_bot_api = MessagingApi(api_client)                    
                    line_bot_api.reply_message_with_http_info(
                        ReplyMessageRequest(
        ...
        elif action == 'confirm_delete_report':
            # 執行刪除報告的操作
            report_name = data.get('report_name')
            pet_name = data.get('p_n')
            ...
            if matching_blobs:
                ...
                if report_files:
                    ...
                    quick_reply = QuickReply(items=quick_reply_items)
                    messages[0].quick_reply = quick_reply
                    send_message_with_selected_pet(user_id, reply_token, f"報告已成功刪除。", messages)
                else:
                    send_message_with_selected_pet(user_id, reply_token, f"報告已成功刪除，目前該寵物 {pet_name} 並沒有報告。")
            else:
                send_message_with_selected_pet(user_id, reply_token, "找不到指定的報告。")

        elif action == 'delete_pet':
            # 顯示確認刪除寵物的Quick Reply
            pet_name = data.get('p_n')
            
            quick_reply=QuickReply(items=[
                QuickReplyItem(action=PostbackAction(label="確定", data=json.dumps({"action": "confirm_delete_pet", "p_n": pet_name}))),
                QuickReplyItem(action=PostbackAction(label="取消", data=json.dumps({"action": "cancel_delete"})))
            ])
            
            message = TextMessage(text=f"您確定要刪除寵物 {pet_name} 嗎？", sender=sender, quick_reply=quick_reply)
            with ApiClient(configuration) as api_client:
                    line_bot_api = MessagingApi(api_client)                    
                    line_bot_api.reply_message_with_http_info(
                        ReplyMessageRequest(
        ...
        elif action == 'confirm_delete_pet':
            # 執行刪除寵物的操作
            pet_name = data.get('p_n')
            ...
            send_message_with_selected_pet(user_id, reply_token, "刪除操作已取消。")
