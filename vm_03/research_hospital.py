...
def get_local_time():  #獲取當前的本地時間（台北時間）
...
def is_hospital_open_now(opening_hours):  #判斷是否營業，取得當前的日期和時間
...
def pet_hospital(user_location, keyword="營業中的動物醫院"):
    ...
    for place in places:
        ...
        print(f"Hospital: {hospital_name}, Open Now: {is_open_now}, Distance: {distance_text}")

        if len(results) >= 3:
            break

    print(f"Total hospitals found: {len(results)}")
    return results
